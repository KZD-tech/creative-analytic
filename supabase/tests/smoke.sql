...
insert into public.campaigns (id, name) values ('smoke-test', 'Smoke Test');
insert into public.benchmarks (campaign_id) values ('smoke-test');
...
select 'smoke-test', id, d::date, d::date, 100, 10000, 6000, 300
  from public.creatives, generate_series('2026-03-01'::date, '2026-03-03'::date, '1 day') d
 where ad_name_key = 'ad one';

insert into public.ad_metrics
  (campaign_id, creative_id, date_start, date_stop, spend, impressions, reach, link_clicks)
select 'smoke-test', id, '2026-03-01', '2026-03-03', 300, 30000, 12000, 900
  from public.creatives where ad_name_key = 'ad one';
...
select 'smoke-test', id, '2026-03-01', '2026-03-03', 500, 40000, 20000, 400
  from public.creatives where ad_name_key = 'ad two';

insert into public.conversions (campaign_id, creative_id, dedupe_key, occurred_at, amount)
select 'smoke-test', id, 'smoke-' || g, timestamptz '2026-03-01 09:00+08' + (g || ' hours')::interval, 50
  from public.creatives, generate_series(1, 6) g
 where ad_name_key = 'ad one';
...
begin
  select spend into one_spend
    from public.creative_performance('smoke-test') where ad_name = 'Ad One';
  ...
  select spend into two_spend
    from public.creative_performance('smoke-test') where ad_name = 'Ad Two';
  if two_spend <> 500 then
    raise exception 'range-only creative should report its range row: got %', two_spend;
  end if;

  select spend into total from public.campaign_summary('smoke-test');
  if total <> 800 then
    raise exception 'campaign summary should be 800, got %', total;
  end if;

  select count(*) into days from public.daily_series('smoke-test');
  if days <> 3 then
  ...
  select revenue into first_day
    from public.daily_series('smoke-test') where day = '2026-03-01';
  ...
  perform public.creative_performance('smoke-test', '2020-01-01', '2020-01-31');
